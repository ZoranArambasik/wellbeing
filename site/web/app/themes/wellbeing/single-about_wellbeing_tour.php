            ...
            <?php if (have_posts()) : while (have_posts()) : the_post(); ?>
               ...
               <?php $next_post = get_previous_post();
                if($next_post) {
                   ...
                   echo '<h4 class="next-wb-post">NEXT: <a rel="prev" href="' . get_permalink($next_post->ID) . '" title="' . $next_title. '" class=" ">'. $next_title . '</a></h4>';
                } ?>
            <?php endif; ?>
            </div>
            <div class="col-md-5 p-0 sidebar-wellbeing-singles">
             <?php include get_theme_file_path('includes/about-wellbeing-sidebar.php'); ?>
            </div>
        </div>
      </div>
    </div>
